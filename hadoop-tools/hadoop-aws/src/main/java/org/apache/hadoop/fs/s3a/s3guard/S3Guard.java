...
final public class S3Guard {
  ...
  public static FileStatus[] dirListingUnion(MetadataStore ms, Path path,
    ...
    // TODO filter out deleted files via PathMetadata#isDeleted()
    ...
    for (FileStatus s : backingStatuses) {
      ...
      // This appears to be a real issue, so:
      // TODO expose an atomic DirListingMetadata#putIfNotPresent()
   ...
   *              dir.
   * @param permission Permissions associated with the directories.
   *                   Currently ignored by s3a and here as well.
   */
  public static void makeDirsOrdered(MetadataStore ms, List<Path> dirs,
    ...
    for (int i = 0; i < dirs.size(); i++) {
      ...
      Path f = dirs.get(i);
      assertQualified(f);
      S3AFileStatus status = new S3AFileStatus(true, isLeaf, f);
  ...
  public static void addMoveDir(MetadataStore ms, Collection<Path> srcPaths,
    ...
    S3AFileStatus dstStatus = new S3AFileStatus(true, true, dstPath);
