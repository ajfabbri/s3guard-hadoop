 ...
 * is certinaly important since ADL fetches
...
final class BufferManager {
  ...
  private BufferManager() {
    guid = PrivateAzureDataLakeFileSystem.getMachineName() + System
        .currentTimeMillis();
  }
  ...
  boolean hasValidDataForOffset(String path, long offset) {
    ...
    if ((offset < buffer.offset) || (offset >= (buffer.offset
        + buffer.data.length))) {
      ADLLogger.log("[" + guid + "]" + path + " cache Miss Expected from " +
          offset + " Available from " + buffer.offset);
      return false;
    }
  ...
  boolean hasData(String path, long offset, int size) {
    ...
    if ((size + offset) > (buffer.data.length + buffer.offset)) {
      ADLLogger.log("[" + guid + "]" + path + " cache Miss Expected from " +
          size + " + Available from " + buffer.data.length);
      return false;
    }
    return true;
  }
  ...
  void add(byte[] data, String path, long offset) {
    ...
    ADLLogger.log("[" + guid + "]" + path + " Update Cache " + offset + " " +
        "Available till " + data.length);
    buffer = new Buffer();
    buffer.data = data;
