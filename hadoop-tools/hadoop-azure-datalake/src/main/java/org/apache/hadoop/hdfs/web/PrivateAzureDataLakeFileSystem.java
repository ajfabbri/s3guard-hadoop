...
import org.apache.hadoop.fs.azure.metrics.AdlFileSystemInstrumentation;
import org.apache.hadoop.fs.azure.metrics.AdlFileSystemMetricsSystem;
...
import org.apache.hadoop.hdfs.web.resources.AppendCaboNoRedirectParam;
import org.apache.hadoop.hdfs.web.resources.BlockSizeParam;
import org.apache.hadoop.hdfs.web.resources.BufferSizeParam;
import org.apache.hadoop.hdfs.web.resources.CreateCaboNoRedirectParam;
...
import org.apache.hadoop.hdfs.web.resources.ReadCaboNoRedirectParam;
...
/**
 * Extended @see SWebHdfsFileSystem API. This class contains Azure data lake specific stability, Reliability and performance
 * improvement.
 * <p>
 * This class is extended to support swebhdfs schema.
 * <p>
 * Configuration supported from Core-site.xml are below
 *
 * <pre>
 *     {@code
 *          <property>
 ...
 *         <value>true</value>
 *         <description>If true then Read ahead implementation kicks in. Default is true.</description>
 *     </property>
 *     }
 * </pre>
 *
 ...
 *         <description>
 *             Define maximum buffer size can be allocated by ADL Webhdfs per process to cache read ahead data.
 *             Default size if not configured is 64MB. Applicable only when ADL.Feature.Override.ReadAhead is set to true.
 *         </description>
 *     </property>
 *     }
 * </pre>
 ...
 *          <value>2</value>
 *          <description>Define maximum concurrent connection can be established to read ahead.
 *          If the data size is <4MB then only 1 read n/w connection is set.
 *          If the data size is >4MB but <16MB then 2  read n/w connection is set. Data >16MB then value set under
 *          the property would take effect. Applicable only when ADL.Feature.Override.ReadAhead is set to true and
 *          buffer size is >16MB.
 *          It is recommended to reset this property if the ADL.Feature.Override.ReadAhead.Max.BufferSize
 *          is <64MB to gain performance.
 *          </description>
 *      </property>
 *     }
 * </pre>
 ...
 *     {@code
 *     <property>
 *         <name>ADL.Feature.Override.GetBlockLocation.Locally.Bundled</name>
 *         <value>true</value>
 *         <description>getFileBlockLocation is constructed locally. Default is true. </description>
 *     </property>
 *     }
 * </pre>
 */
public class PrivateAzureDataLakeFileSystem extends SWebHdfsFileSystem {
    ...
    /**
     * Process wide threadpool for data lake file system
     * Threads need to be daemon so that they dont prevent the process from exiting
     */
    ...
    public synchronized void initialize(URI uri, Configuration conf
        ...
        overrideOwner = getConf().getBoolean(ADLConfKeys.ADL_DEBUG_OVERRIDE_LOCAL_USER_AS_OWNER,
                ADLConfKeys.ADL_DEBUG_SET_LOCAL_USER_AS_OWNER_DEFAULT);

        ADLFeatureList.setFeatureRedirectOff(getConf().getBoolean(ADLConfKeys.ADL_FEATURE_REDIRECT_OFF,
                ADLConfKeys.ADL_FEATURE_REDIRECT_OFF_DEFAULT));

        ADLFeatureList.setFeatureCacheFileStatus(getConf().getBoolean(ADLConfKeys.ADL_FEATURE_CACHE_FILE_STATUS,
                ADLConfKeys.ADL_FEATURE_CACHE_FILE_STATUS_DEFAULT));

        ADLFeatureList.setFeatureCacheFileStatusDuration(getConf().getInt(ADLConfKeys.ADL_FEATURE_CACHE_FILE_STATUS_DURATION,
                ADLConfKeys.ADL_FEATURE_CACHE_FILE_STATUS_DURATION_DEFAULT));

        ADLFeatureList.setFeatureGetBlockLocationLocallyBundled(getConf().getBoolean(ADLConfKeys.ADL_FEATURE_GET_BLOCK_LOCATION_LOCALLY_BUNDLED,
                ADLConfKeys.ADL_FEATURE_GET_BLOCK_LOCATION_LOCALLY_BUNDLED_DEFAULT));

        ADLFeatureList.setFeatureConcurrentReadWithReadAhead(getConf().getBoolean(ADLConfKeys.ADL_FEATURE_CONCURRENT_READ_WITH_READ_AHEAD,
        ...
        maxBufferSize = getConf().getInt(ADLConfKeys.ADL_FEATURE_CONCURRENT_READ_WITH_READ_AHEAD_BUFFER_SIZE,
                ADLConfKeys.ADL_FEATURE_CONCURRENT_READ_WITH_READ_AHEAD_BUFFER_SIZE_DEFAULT);

        maxConcurrentConnection = getConf().getInt(ADLConfKeys.ADL_FEATURE_CONCURRENT_READ_WITH_READ_AHEAD_MAX_CONCURRENT_CONNECTION,
                ADLConfKeys.ADL_FEATURE_CONCURRENT_READ_WITH_READ_AHEAD_MAX_CONCURRENT_CONNECTION_DEFAULT);

        OAuth2ConnectionConfigurator connectionConfigurator = new OAuth2ConnectionConfigurator(conf);
        super.connectionFactory = new PrivateDebugUrlConnectionFactory(connectionConfigurator, objectGuid, conf);
        ...
        AdlFileSystemMetricsSystem.registerSource(metricsSourceName, sourceDesc,
                instrumentation);
    }
    ...
    /**
     * Need to override default getHomeDirectory implementation due to HDFS-8542 causing MR jobs to fail in initial
     * phase. Constructing home directory locally is fine as long as hadoop local user name and cabo user name relation
    ...
    /**
     * Azure data lake does not support user configuration for data replication hence not leaving system to query on
     * azure data lake.
     * <p>
     * Stub implementation
     *
     ...
     * @return True hard coded since ADL file system does not support replication configuration
     * @throws IOException No exception would not thrown in this case however aligning with parent api definition.
     */
    @Override
     ...
     * <p>
     * Removes cached FileStatus entry to maintain latest information on the FileStatus instance
     *
    ...
    public final void setTimes(final Path p, final long mtime, final long atime
        ...
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePath = p.isAbsoluteAndSchemeAuthorityNull() ? getUri() + p.toString() : p.toString();
            fileStatusCacheManager.removeFileStatus(new Path(filePath));
        }
    ...
    /**
     * Avoid call to Azure data lake backend system. Look in the local cache if FileStatus from the previous call has
     * already been cached.
     * <p>
    ...
    public FileStatus getFileStatus(Path f) throws IOException {
        ...
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePath = f.isAbsoluteAndSchemeAuthorityNull() ? getUri() + f.toString() : f.toString();
            status = fileStatusCacheManager.getFileStatus(filePath);
        }
        ...
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePath = f.isAbsoluteAndSchemeAuthorityNull() ? getUri() + f.toString() : f.toString();
            if (fileStatusCacheManager.getFileStatus(filePath) == null) {
                fileStatusCacheManager.putFileStatus(status);
            }
        }

        if (overrideOwner) {
            FileStatus proxiedStatus = new FileStatus(status.getLen(), status.isDirectory(),
     ...
     *
     * @param f         File/Folder path
     * @param recursive true if the contents within folder needs to be removed as well
    ...
    public boolean delete(Path f, boolean recursive) throws IOException {
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePath = f.isAbsoluteAndSchemeAuthorityNull() ? getUri() + f.toString() : f.toString();
            FileStatus fs = fileStatusCacheManager.getFileStatus(filePath);
    ...
    public boolean rename(final Path src, final Path dst) throws IOException {
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePathSrc = src.isAbsoluteAndSchemeAuthorityNull() ? getUri() + src.toString() : src.toString();
            String filePathDst = dst.isAbsoluteAndSchemeAuthorityNull() ? getUri() + dst.toString() : dst.toString();
            FileStatus fsSrc = fileStatusCacheManager.getFileStatus(filePathSrc);
            FileStatus fsDst = fileStatusCacheManager.getFileStatus(filePathDst);

            if ((fsSrc != null && !fsSrc.isFile()) || (fsDst != null && !fsDst.isFile())) {
                fileStatusCacheManager.clear();
            } else {
                fileStatusCacheManager.removeFileStatus(new Path(filePathSrc));
                fileStatusCacheManager.removeFileStatus(new Path(filePathDst));
            }
        }
     ...
     * @return FSDataOutputStream OutputStream on which application can push stream of bytes
     * @throws IOException For any system exception
     */
    @Override
     ...
     * @param srcs List of sources to be concatinated. ADL concatinate in the same order passed as parameter.
     * @throws IOException For any system exception
     */
    @Override
    public void concat(final Path trg, final Path[] srcs) throws IOException {
        if (ADLFeatureList.isFeatureCacheFileStatus()) {
            String filePath = trg.isAbsoluteAndSchemeAuthorityNull() ? getUri() + trg.toString() : trg.toString();
            fileStatusCacheManager.removeFileStatus(new Path(filePath));
            for (int i = 0; i < srcs.length; ++i) {
                filePath = srcs[0].isAbsoluteAndSchemeAuthorityNull() ? getUri() + srcs[0].toString() : srcs[0].toString();
                fileStatusCacheManager.removeFileStatus(new Path(filePath));
            }
        }
    ...
    /**
     * Create call semantic is handled differently in case of ADL. Create semantics is translated to Create/Append
     * semantics.
     * 1. No dedicated connection to server.
     * 2. Buffering is locally done, Once buffer is full or flush is invoked on the by the caller. All the pending
     * data is pushed to ADL as APPEND operation code.
     * 3. On close - Additional call is send to server to close the stream, and release lock from the stream.
     * <p>
     * Necessity of Create/Append semantics is
     * 1. ADL backend server does not allow idle connection for longer duration. In case of slow writer scenario,
     * observed connection timeout/Connection reset causing occasional job failures.
     * 2. Performance boost to jobs which are slow writer, avoided network latency
     * 3. ADL equally better performing with multiple of 4MB chunk as append calls.
     *
     * @param f File path
     * @param permission Access perfrmission for the newly created file
     * @param overwrite Remove existing file and recreate new one if true otherwise throw error if file exist
    ...
    public FSDataOutputStream create(final Path f, final FsPermission permission,
        ...
        return new FSDataOutputStream(new BatchAppendOutputStream(f, bufferSize,
                ...
                new BlockSizeParam(blockSize), new ADLVersionInfo()), statistics) {
        };
    }
    ...
    public FSDataOutputStream createNonRecursive(final Path f,
        ...
        return new FSDataOutputStream(new BatchAppendOutputStream(f, bufferSize,
                ...
                new BufferSizeParam(bufferSize),
                new ReplicationParam(replication),
                new LeaseParam(leaseId),
                new BlockSizeParam(blockSize), new ADLVersionInfo()), statistics) {
        };
    }

    /**
     * Since defined as private in parent class, redefined to pass through Create api implementation.
     *
     * @param permission
    ...
    /**
     * Open call semantic is handled differently in case of ADL. Instead of network stream is returned to the user,
     * Overridden FsInputStream is returned.
     * <p>
     * 1. No dedicated connection to server.
     * 2. Process level concurrent read ahead Buffering is done, This allows data to be available for caller quickly.
     * 3. Number of byte to read ahead is configurable.
     * <p>
     * Advantage of Process level concurrent read ahead Buffering semantics is
     * 1. ADL backend server does not allow idle connection for longer duration. In case of slow reader scenario,
     * observed connection timeout/Connection reset causing occasional job failures.
     * 2. Performance boost to jobs which are slow reader, avoided network latency
     * 3. Compressed format support like ORC, and large data files gains the most out of this implementation.
     * <p>
     * Read ahead feature is configurable.
     *
     * @param f File path
     * @param buffersize Buffer size
     * @return FSDataInputStream InputStream on which application can read stream of bytes
     * @throws IOException when system error, internal server error or user error
     */
    @Override
    public FSDataInputStream open(final Path f, final int buffersize
        ...
        ADLLogger.log("statistics report Time " + (System.currentTimeMillis() - statContructionTime));

        final HttpOpParam.Op op = GetOpParam.Op.OPEN;
        // use a runner so the open can recover from an invalid token
        FsPathConnectionRunner runner = null;

        if (ADLFeatureList.isFeatureConcurrentReadWithReadAhead()) {
            long urlContructionTime = System.currentTimeMillis();
            URL url = this.toUrl(op, f, new BufferSizeParam(buffersize), new ReadCaboNoRedirectParam(true), new ADLVersionInfo());
            ADLLogger.log("URL Construction Time " + (System.currentTimeMillis() - urlContructionTime));
            ...
            ADLLogger.log("BatchByteArrayInputStream Construction Time " + (System.currentTimeMillis() - bbContructionTime));

            long finContructionTime = System.currentTimeMillis();
            FSDataInputStream fin = new FSDataInputStream(bb);
            ADLLogger.log("FSDataInputStream Construction Time " + (System.currentTimeMillis() - finContructionTime));
            return fin;
        } else {
            if (ADLFeatureList.isFeatureRedirectOff()) {
                long urlContructionTime = System.currentTimeMillis();
                runner = new FsPathConnectionRunner(ADLGetOpParam.Op.OPEN, f, new BufferSizeParam(buffersize),
                        new ReadCaboNoRedirectParam(true), new ADLVersionInfo());
                ADLLogger.log("Runner Construction Time " + (System.currentTimeMillis() - urlContructionTime));
            } else {
    ...
    /**
     * On successful response from the server, @see FileStatusCacheManger is updated with FileStatus objects.
     *
     * @param f File/Folder path
    ...
    public FileStatus[] listStatus(final Path f) throws IOException {
        ...
        for (int i = 0; i < fileStatuses.length; i++) {
            if (ADLFeatureList.isFeatureCacheFileStatus()) {
                fileStatusCacheManager.putFileStatus(fileStatuses[i]);
            }

            if (overrideOwner) {
                fileStatuses[i] = new FileStatus(fileStatuses[i].getLen(), fileStatuses[i].isDirectory(),
                        fileStatuses[i].getReplication(), fileStatuses[i].getBlockSize(), fileStatuses[i].getModificationTime(),
                        fileStatuses[i].getAccessTime(), fileStatuses[i].getPermission(), userName
                        ,
                        "hdfs", fileStatuses[i].getPath());
            }
        }
        return fileStatuses;
    }
    ...
    public BlockLocation[] getFileBlockLocations(final FileStatus status,
        ...
        if (ADLFeatureList.isFeatureGetBlockLocationLocallyBundled()) {
            if ((offset < 0) || (length < 0)) {
                throw new IllegalArgumentException("Invalid start or len parameter");
            }

            if (status.getLen() < offset) {
                if (ADLFeatureList.isLogEnabled()) {
                    ADLLogger.log("getFileBlockLocations : Returning 1 block");
                }
                return new BlockLocation[0];
            }
            final String blobLocationHost = "localhost";
            ...
            int numberOfLocations = (int) (length / blockSize)
            ...
            if (ADLFeatureList.isLogEnabled()) {
                ADLLogger.log("getFileBlockLocations : Returning " + locations.length + " Blocks");
            }
    ...
    class BatchAppendOutputStream extends OutputStream {
        ...
        public BatchAppendOutputStream(Path path, int bufferSize, Param<?, ?>... param) throws IOException {
            ...
            try {
                if (ADLFeatureList.isFeatureRedirectOff()) {
                    CreateCaboNoRedirectParam skipRedirect = new CreateCaboNoRedirectParam(true);
                    Param<?, ?>[] tmpParam = ADLFeatureList.isFeatureFlushWhenEOF() ? new Param<?, ?>[param.length + 2] : new Param<?, ?>[param.length + 1];
                    System.arraycopy(param, 0, tmpParam, 0, param.length);
                    tmpParam[param.length] = skipRedirect;
                    if (ADLFeatureList.isFeatureFlushWhenEOF()) {
        ...
        public final synchronized void write(int b) throws IOException {
            ...
            // Statistics will get incremented again as part of the batch updates, decrement here to avoid double value
            if (statistics != null) {
        ...
        public final synchronized void write(byte[] buf, int off, int len) throws IOException {
            ...
            if (localLen >= data.length) {
                ...
                // Keep committing data until we have less than our internal buffers length left
                do {
                    try {
            ...
            // At this point, we have less than data.length left to copy from users buffer
            ...
            if (localLen > 0) {
                // Simply copy the remainder from the users buffer into our internal buffer
        ...
        public final synchronized void close() throws IOException {
            ...
            if (hadError) {
                // No point proceeding further since the error has occurered and stream would be required to upload again.
                return;
            } else {
            ...
            if (ADLFeatureList.isFeatureRedirectOff()) {
                // If we didn't flush anything from our internal buffer, we have to call the service again
        ...
        private void commit(byte[] buffer, int off, int len, boolean eof) throws IOException {
            OutputStream out = null;
            try {
                if (ADLFeatureList.isFeatureRedirectOff()) {
                    AppendCaboNoRedirectParam skipRedirect = new AppendCaboNoRedirectParam(true);
                    Param<?, ?>[] tmpParam = ADLFeatureList.isFeatureFlushWhenEOF()
                            ? new Param<?, ?>[parameters.length + 3] : new Param<?, ?>[parameters.length + 1];
                    System.arraycopy(parameters, 0, tmpParam, 0, parameters.length);
                    tmpParam[parameters.length] = skipRedirect;
                    if (ADLFeatureList.isFeatureFlushWhenEOF()) {
                        tmpParam[parameters.length + 1] = new ADLFlush(eof);
                ...
                if (buffer != null) {
                    fileStatusCacheManager.removeFileStatus(fsPath);
        ...
        private void waitForOutstandingFlush() throws IOException {
            if (flushTask != null) {
                try {
                    ...
                    // Wrap the ExecutionException in an IOException for callers can only handle IOException
                    throw new IOException(ex);
                } finally {
                    flushTask = null;
                }
    ...
    /**
     * Read data from backend in chunks instead of persistent connection. This is to avoid slow reader causing socket
     * timeout.
     * <p>
     ...
     * </pre>
     */
    class BatchByteArrayInputStream extends FSInputStream {
        ...
        public BatchByteArrayInputStream(URL url, Path p, int bufferSize, int concurrentConnection) throws IOException {
            this.runner = url;
            fsPath = p;
        ...
        @Override
        public final void readFully(long position, byte[] buffer, int offset, int length) throws IOException {
            if (streamIsClosed) {
                throw new IOException("Stream already closed");
            }
            long startTime = System.currentTimeMillis();
            super.readFully(position, buffer, offset, length);
            ADLLogger.log("ReadFully1 Time Taken : " + (System.currentTimeMillis() - startTime));
            if (statistics != null) {
        ...
        public final int read(byte[] b, int off, int len) throws IOException {
            ...
            if (requestedBytes <= availableBytes) {
                ...
                ADLLogger.log("AC - [BufferOffset : " + bufferOffset + " CurrentFileSite : " + currentFileOffset + "] Offset : "
                        + off + " Length : " + len + " Read Time Taken : " + (System.currentTimeMillis() - startTime));
                try {
                    System.arraycopy(data, bufferOffset, b, off, len);
                } catch (ArrayIndexOutOfBoundsException e) {
                    ADLLogger.log("ArrayIndexOutOfBoundsException AC - [BufferOffset : " + bufferOffset + " CurrentFileSite : "
                            + currentFileOffset + "] Offset : " + off + " Length : " + len + " Read Time Taken : "
                ...
                ADLLogger.log("HC - [BufferOffset : " + bufferOffset + " CurrentFileSite : " + currentFileOffset + "] Offset : "
                        + off + " Length : " + len + " Read Time Taken : " + (System.currentTimeMillis() - startTime));
        ...
        public final long fill(final long off) throws IOException {
            ...
            ADLLogger.log("Buffer Refill Time Taken : " + (System.currentTimeMillis() - startTime));
            return validDataHoldingSize;
        }

        private int getSplitSize(int size) {
            int splitSize = 1;
            if (size <= SIZE4MB) {
                splitSize = 1;
            } else {
                splitSize = maxConcurrentConnection;
            }
            return splitSize;
        }
        ...
        public final void seek(long pos) throws IOException {
            ...
            ADLLogger.log("SEEK  : " + pos + " Available " + currentFileOffset + " Count " + availableBytes);
        ...
        public final int fillDataConcurrently(byte[] byteArray, int length, long globalOffset, int splitSize) throws IOException, InterruptedException {
        ...
        public final void close() throws IOException {
            ...
            //need to cleanup the above code the stream and connection close doesnt happen here
            //flag set to mark close happened, cannot use the stream once closed
            streamIsClosed = true;
        }

        private int fillUpData(byte[] buffer, int offset, int length, long globalOffset) throws IOException {
            int totalBytesRead = 0;
            final URL offsetUrl = new URL(runner + "&" + new OffsetParam(String.valueOf(globalOffset)) + "&"
