...
/**
 * Extended version HttpUrlConnection allows request to be monitor for performance and instrument with tracking header
 * part of the request.
 */
public final class PrivateDebugUrlConnection extends HttpURLConnection {
     ...
     * @param uniqueId Unique id per webhdfs file system instance. For debugging/performance purpose
     * @param connectionStartTime Connection initiate start time from ConnectionFactory. For debugging/performance purpose
     * @param connectionEndTime Connection initiate end time from ConnectionFactory. For debugging/performance purpose
     */
    protected PrivateDebugUrlConnection(URL url, String uniqueId, long connectionStartTime, long connectionEndTime) {
        super(url);
        this.guid = uniqueId;
    ...
    private void debugInfo() {
        ...
        obj.append(",\"TotalTimeSpendInMs\" : " + (endTimeInLong - startTime));
        obj.append(",\"ResponseCode\" : " + responseCode);
        obj.append(",\"Response\" : \"" + responseMessage + "\"");
        obj.append(",\"ServerPerf\" : \"" + getHeaderField("server-perf") + "\"");
        obj.append(",\"Machine\" : \"" + getHeaderField("x-origin-server") + "\"");
        obj.append(",\"RequestStartTime\" : \"" + sdf.format(startTimeDate) + "\"");
        obj.append(",\"SSLHandshakeCompleteTime\" : \"" + sdf.format(connectionEstablishedTimeDate) + "\"");
        obj.append(",\"SSLHandshakeCompleteTimeInMs\" : " + (connectionEstablishedTime - startTime));
        obj.append(",\"RequestEndTime\" : \"" + sdf.format(new Date(endTimeInLong)) + "\"");
        obj.append(",\"URL\" :  \"" + getURL() + "\"");
        obj.append(",\"WebhdfsObjectId\" : \"" + guid + "\"");
        obj.append(",\"OAuthTokenRequest\" : \"" + sdf.format(openConnectionStartTime) + "\"");
        obj.append(",\"OAuthTokenResponse\" : \"" + sdf.format(openConnectionEndTime) + "\"");
        obj.append(",\"OAuthRefreshTokenTimeInMs\" : " + (openConnectionEndTime - openConnectionStartTime));
        obj.append(",\"Host\" : \"" + PrivateAzureDataLakeFileSystem.getMachineName() + "\"");
