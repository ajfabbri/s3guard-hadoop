...
public class TestGetFileStatus extends TestBaseSetup {
  ...
  public void getFileStatusReturnsAsExpected()
    ...
    try {
      mockWebHDFSServerClient.when(fileSystemRequest, exactly(1)).respond(
          response().withStatusCode(HttpStatus.SC_OK)
              .withHeaders(CONTENT_TYPE_APPLICATION_JSON)
              .withBody(TestADLResponseData.getGetFileStatusJSONResponse()));
      ...
      FileStatus ls = fs.getFileStatus(new Path("/test1/test2"));
