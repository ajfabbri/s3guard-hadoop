...
import org.apache.hadoop.fs.adl.TestAdlFileSystem;
...
public class TestBaseSetup {
  ...
  protected final static Header CONTENT_TYPE_APPLICATION_JSON = new Header(
      "Content-Type", "application/json");
  protected final static String AUTH_TOKEN = "0123456789abcdef";
  protected final static Header AUTH_TOKEN_HEADER = new Header("AUTHORIZATION",
  ...
  protected HttpRequest getOAuthServerMockRequest(
      MockServerClient mockServerClient) {
    HttpRequest expectedRequest = request().withMethod("POST").withPath(
        "/refresh")
        // TODO: rewrite with JsonBody now that no longer relying on
        // unirest...
        .withBody("client_secret=credential&grant_type=client_credentials"
    ...
    HttpResponse resp = response().withStatusCode(HttpStatus.SC_OK)
        .withHeaders(CONTENT_TYPE_APPLICATION_JSON)
        .withBody(jsonObject.toString());

    mockServerClient.when(expectedRequest, unlimited()).respond(resp);

    return expectedRequest;
  }

  public Configuration getADLConfiguration() {
